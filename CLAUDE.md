...
| 28 | Full Test Suite Completion | Every remaining Test Plan v1.0 §14 hard blocker this phase's own Scope IN names, closed for real — `docs/test-plan-compliance.md` (new) maps every §14 item to its exact test, including two honestly-reported gaps this phase found rather than silently marked PASS: RECLAIM-04's own already-recorded 269-deadlock finding (Phase 17, "zero deadlocks" not currently true) and CONC-06/PERF-01-03 (genuinely unbuilt, deferred to Phase 29). IDEM-07 (`tests/bookings/test_idempotency.py`) forces a genuine exception at the exact point a process kill mid-write would land — inside `run_idempotent_write`'s own shared `transaction.atomic()`, after the booking INSERT has run but before the outcome-record UPDATE commits — proving Postgres's own atomicity (not application retry logic) is what makes "a booking with no key record" structurally impossible; ground truth confirms neither survives the forced crash. IDEM-08 reframes IDEM-02's already-proven mechanism around the ACTUAL product requirement: the first response is never inspected at all (simulating a proxy-level drop), only its side effect (a committed booking, read via an independent query) is used as ground truth, and the retry with the identical key shows the user their own booking, never a fresh `slot_unavailable`. `tests/test_failure_modes.py` (new) — FAIL-01 (a connection-level failure, i.e. no SQLSTATE at all — the genuine failover shape — forced on the FIRST `Booking.objects.create()` call only via a call-counter closure, through the REAL `POST /api/v1/bookings` endpoint: 503, `Retry-After`, `cause="failover"`, nothing recorded under the key since the outcome was genuinely unknown, then an unambiguous 201 on retry) and FAIL-02 (genuine reproduction, not simulation — the exact real-blocking-row technique `test_services.py`'s existing lock-timeout test already established, extended through the real HTTP endpoint: 503 explicitly not 500 not 409, `cause="lock_contention"`, unambiguous retry once the row releases). `kairos/core/replica.py` (new) is FAIL-03's seam — per this phase's own explicit instruction, NOT real replica infrastructure: `current_replica_lag_seconds()` honestly returns `None` (no replica exists; Phase 30 builds one) and `select_read_source(lag_seconds, threshold)` is the actual degradation rule (unknown or over-threshold lag always falls back to `"primary"`, reported honestly via `data_freshness`, never silently stale) — proven both as a direct unit (`tests/test_replica.py`) and through the real `GET /resources/{id}/availability` endpoint with a monkeypatched lag function (`tests/resources/test_views.py`); `kairos/resources/views.py`'s hardcoded `"primary"` literal is now this function's real (if, today, always-identical) output. Three genuine §10 functional-matrix gaps found by a row-by-row audit of all 67 rows (not the summary line's implied 62, recounted twice) against the running suite and closed: the 365-day horizon's TRUE boundary (`tests/bookings/test_views.py`, freezing `timezone.now()` so `start == now + 365 days` is deterministic, not the existing test's 5-minutes-inside proxy for it), a recurring series with zero future occurrences (`tests/bookings/test_recurring_series.py` — every prior series-cancel test had at least one future/uncancelled occurrence), and the six-check response content asserted for an `operations` caller specifically, not only `system_admin` (`tests/test_admin_checks.py`). CONC-01's full-scale exercise (`tests/concurrency/test_conc_01_full_scale.py`, staging tier, `--ignore`d in CI's `concurrency` job matching RECLAIM-04's own precedent) was ACTUALLY RUN, not merely written, per this project's own RECLAIM-04-set discipline: **100/100 runs at N=200 each produced exactly one ground-truth-verified success** (111 total attempts across all 100 runs — 6 runs needed a retry after a zero-success attempt, maximum 5 attempts in any single run, well inside the `MAX_ROUND_ATTEMPTS=10` budget; 10,348×23P01, 10,798×57014, 727×40P01, 227×55P03 across all non-success outcomes, zero unexplained), and the **separate N=500 escalation round also produced exactly one success, zero retries needed** (374×57014, 116×55P03, 9×40P01, zero 23P01 — this single very-high-contention round never resolved via the constraint's own conflict path at all). Full transcript and SQLSTATE breakdown in `docs/test-plan-compliance.md`. Coverage reporting wired into CI (`pytest-cov`, `--cov-fail-under=90`, XML uploaded as a build artifact) — 93% measured baseline at the time this was wired up, 90% the documented target (deliberate headroom, not equal to the baseline and not 100%; two files sit at 0% by design, both thin CLI wrappers around already-unit-tested functions). Full suite: 375 tests (all backend tests excluding the concurrency directory, which has its own separate CI job and separate staging-tier files), zero regressions, ruff/mypy both clean. | Pending (on branch `phase-28-full-test-suite`) |
| 29 | Performance & Load Testing | Real numbers against every NFR target, from a real PRD-A1-scale dataset, closing PERF-01–03/CONC-06/CONC-06a (all explicitly out of Phase 28's own scope) and resolving RECLAIM-04's own open item rather than deferring it a third time. `manage.py seed_perf_data` (new) seeds `kairos_dev` — 2,000 users, 301 resources (5 near-fully-booked "dense" resources for PERF-02(b); 280 representative-density "moderate" resources, PRD A1's own "including held and cancelled rows," for PERF-02(a); 15 resources kept empty for PERF-01's guaranteed-free writes; 1 kept empty for CONC-06), 17,859 confirmed / 286 held / 681 cancelled bookings — namespaced (`@perf.kairos.local` / `PERF ` prefixes) so `--reset` can find and remove exactly what it created without touching real `kairos_dev` data. `kairos/settings/perf.py` (new) is the one committed settings addition — `RATE_LIMIT_ENABLED=False` (a synthetic single-machine load-testing harness looks exactly like the per-IP fairness limiter's target and isn't PRD A1's real many-distinct-users traffic) and a widened `KAIROS_SESSION_TOKEN_TTL_SECONDS` (a long escalation run needs tokens minted once at start to outlive it). `scripts/perf/` (new, throwaway like `scripts/spike/` — never becomes application code) — `common.py` (real HTTP via stdlib `urllib`, real session tokens via the actual `issue_session_token`, `threading.Barrier`-released true simultaneity for every "concurrent" measurement, the identical discipline `tests/concurrency/harness.py` already established applied to real HTTP instead of raw psycopg), `perf_01_booking_write.py`, `perf_02_availability_read.py`, `perf_03_waitlist_dispatch.py`, `conc_06_escalation.py`. ⚠️ **Real, load-bearing finding, not just measurement noise**: the first attempt, `manage.py runserver`, showed availability-read P95 growing roughly LINEARLY with concurrent request count (150–200ms serial → >2s at N=20) — diagnosed, not assumed: a single serial request stayed fast, ruling out "the query is slow"; swapping to `waitress` (a real multi-threaded production WSGI server, installed ad hoc, never committed) showed the IDENTICAL pattern, ruling out "the dev server specifically is bad"; `gunicorn` (real multi-PROCESS, sidesteps the GIL entirely) cannot run on Windows at all (`ModuleNotFoundError: No module named 'fcntl'`) — a throwaway Linux container (the EXISTING `backend/Dockerfile`, built for the Celery worker, `CMD` swapped to `gunicorn` for this measurement only, never touching `infra/docker-compose.yml`) substantially improved every number (PERF-01 spike P95 7.4s → 3.2s; CONC-06's N=500 step went from 232 outright-refused connections to zero). Conclusion: this codebase's per-request cost is measurably CPU-bound (GIL-serialized under any single-process server) at least as much as I/O-bound — worth carrying into Phase 30's own deployment-platform choice (favor multiple worker PROCESSES, not just threads). A SECOND, fully-understood artifact: 232/500 connections outright refused (`WinError 10061`) under `waitress` on Windows at N=500 traced to CPython's `select()`-based socket multiplexing hitting Windows' well-documented ~512-socket `FD_SETSIZE` ceiling — a test-CLIENT/local-server platform limitation, unrelated to Postgres or the GiST index; zero such refusals on the Linux/gunicorn rig. Every number reported uses the Linux/gunicorn rig, not the raw Windows numbers. **Real results** (`docs/performance-baseline.md`, full methodology and every number): PERF-01 steady PASS (P95 78.4ms, target 300ms); spike FAIL under EVERY serving setup tested, including the multi-process container (P95 3209.8ms, still ~10.7x over target, vs. ~7.4s single-process — a real, substantial improvement, not a resolution) — every one of 200 spike writes still succeeded (201), a latency finding, not a correctness gap, but explicitly NOT recorded as resolved: Phase 30 is required (not merely invited) to (1) deploy with real multi-process serving as the actual production configuration and (2) re-run this specific test against it before either a PASS or a FAIL claim is defensible. PERF-02(a) ordinary-density PASS (P95 375.6ms, target 500ms), (b) near-fully-booked/92-day-bound FAIL (P95 1933.8ms) — a single SERIAL request against the same dense resource completes in 130–200ms, meaning the cost under concurrency is dominated by Python-side JSON-serializing ~2,040 `busy_blocks` entries per request, a cost proportional-but-not-eliminated by better serving (unlike PERF-01's spike, a real deployment re-run will not make this finding moot). **Explicit decision recorded** (matching RECLAIM-04's own rigor, `docs/performance-baseline.md`): ACCEPT for go-live as a documented, scoped limitation (narrow trigger condition, no correctness/safety impact, the fix is a real Spec v1.0 §5.7 contract change needing its own design pass) but flagged HIGH PRIORITY given RFC v1.0 §7.2's own framing of availability reads as the system's highest-traffic endpoint — pagination on `busy_blocks` is the named recommended fix, due before Phase 30 sign-off if a real resource is expected to reach this density early, an immediate fast-follow otherwise. PERF-03 both (a) isolated and (b) 50-cancellation-burst PASS with large margin (P95 33.9ms / 179.4ms, target 5000ms) — dispatch runs in the Celery worker, a SEPARATE process from the web server under test, unaffected by the other two tests' CPU-sharing findings; methodology note: measured to the `WaitlistOffer` row's `created_at` (the closest externally-observable proxy for "enqueued" — `NotificationLog` is written only at DELIVERY time, by the worker, not at enqueue), documented explicitly since it's a real interpretive choice. CONC-06 (informational, not a gate — RFC v1.0 §18's own framing): writes/sec climbs 18.5→66.0→72.8/sec from N=10→25→50 then PLATEAUS at 53.7→57.0→61.3/sec from N=100→250→500 while P95 keeps climbing 539ms→...→7639ms — **named inflection point: N≈50 writers** on one hot resource, the textbook saturation shape; explicitly flagged as this specific 8-vCPU everything-on-one-machine rig's own ceiling, expected to sit higher on real, dedicated production resources. **CONC-06a (the actual hard gate): PASS** — 935/935 writes across all six steps returned 201, zero unexpected errors, on the Linux/gunicorn rig (the Windows/waitress run's 232 refusals were the platform artifact above, re-run and superseded). RECLAIM-04 (Phase 17's own already-recorded 269-deadlock finding) was RE-RUN at the identical full scale (200×50) to get current numbers, not carried forward unverified a third time: **221 deadlocks across 22/50 runs (44%)** — a modest decrease from Phase 17, read as run-to-run variance, not a systematic fix; safety held PERFECTLY both times, across 20,000 combined raw attempts (never more than one success per round). Explicit decision recorded (`docs/performance-baseline.md`, five reasons stated in full): **ACCEPT the current deadlock rate as a known, documented, retryable liveness cost — no locking-strategy change before Phase 30** (zero safety violations twice over; every deadlock already resolves as a proper documented 503+Retry-After, not silently; the scenario — 200 SIMULTANEOUS identical-slot writers — is more extreme than PRD A1's own "spiky, concentrated" scale assumption and Test Plan already tiers it staging-only; a narrower locking strategy is a real architecture change with its own risk for a scenario already excluded from the release gate; the rate held roughly steady, not growing, across four intervening phases of write-path complexity). The GiST write-throughput alert Rollout v1.0 §6 left deliberately unset in Phase 21 ("Set from CONC-06's real characterization data — not invented here") is real now: `AlertKey.GIST_WRITE_THROUGHPUT` (new, migration `0016_gist_write_throughput_alert_key.py` — the identical `RemoveConstraint`/`AlterField`/`AddConstraint` shape already used for `NotificationType.SERIES_OWNER_DEACTIVATED`, Phase 19), `BOOKING_WRITE_P95_ALERT_THRESHOLD_MS=1500` (`kairos/core/constants.py` — sits between CONC-06's own N=50 "still healthy" (666ms) and N=100 "already saturated" (1760ms) readings, explicitly flagged as PROVISIONAL/synthetic-data-derived, matching Rollout v1.0 §9's own "validated or revised... Stage 2 onward" language), a new SEV-3 block in `kairos.core.alerting.evaluate_alerts` reading the SAME live `p95_duration_ms(metric_type=BOOKING_WRITE)` the admin dashboard already computes (Phase 21) — no new aggregation mechanism. Two new tests in `tests/test_alerting.py` (fires over threshold and reaches a real email; does not false-alarm under it, the same "prove the negative too" discipline `offer_cascade`'s own false-alarm test already established). `docs/test-plan-compliance.md` updated in place (kept as ONE living document across Phases 28 and 29, not a second file) — PERF-01–03/CONC-06/CONC-06a rows closed with real numbers, RECLAIM-04's row updated from "open" to "resolved, with an explicit decision." Full suite: 377 tests (375 + 2 new alerting tests), zero regressions, ruff/mypy both clean; all CI-tier concurrency tests re-run clean. | Pending (on branch `phase-29-performance-load`) |

## Current Phase In Progress

None. Phase 29 is complete pending review and merge. Phase 30 is next.
...
something does. No real replica routing yet (Phase 30 stands one up) — `data_freshness` is
computed via `kairos.core.replica.select_read_source` (Phase 28) rather than a hardcoded
literal now, but `current_replica_lag_seconds()` honestly returns `None` (no replica
configured), so the computed value is still always `"primary"` today, byte-identical to the
prior hardcoded behavior — only the DEGRADATION LOGIC downstream of a real measurement is new
and proven (Test Plan FAIL-03; see `docs/test-plan-compliance.md`). The audit trail covers
...
write paths and real HTTP tests. IDEM-07/08 (fault injection — process kill mid-transaction,
proxy-level response drop) are real now (Phase 28) —
`tests/bookings/test_idempotency.py`'s last two tests, both proven through the real
`POST /api/v1/bookings` endpoint, not just at the service layer. Idempotency coverage on
waitlist join/offer confirm arrives with those
...
reason not every test needs converting now. CONC-01's full 100-run + N=500 escalation is real
now (Phase 28) — actually run once, not merely written: 100/100 runs each produced exactly one
ground-truth-verified success (111 total attempts, 6 runs needed a retry, max 5 attempts in any
one run), and the separate N=500 escalation round also produced exactly one success with zero
retries — see `docs/test-plan-compliance.md` for the full SQLSTATE breakdown. CONC-06
(throughput characterization) remains deferred to Phase 29. CI only runs the 10-run CI-tier
reduction for every CONC test, plus the new `test_conc_01_full_scale.py` is `--ignore`d there
(staging tier, run manually before a release). The throwaway spike table
...
| IDEM-07 (Phase 28) forces a genuine exception at the outcome-record `UPDATE` inside `run_idempotent_write`'s own shared `transaction.atomic()` — not a literal OS process kill, and not a new fault-injection framework | Postgres cannot distinguish a killed process from an exception raised inside an open transaction — both trigger the identical ROLLBACK of everything the transaction did, including statements that already executed successfully (the booking INSERT). This is what `run_idempotent_write`'s OWN docstring already claims (Phase 5's "the key claim, the write, and the outcome record all share ONE transaction") — IDEM-07 needed only a way to interrupt at the right POINT, not new machinery, since Django's own `transaction.atomic()` already provides the guarantee under test | `tests/bookings/test_idempotency.py` (`test_idem_07_process_kill_mid_transaction_never_leaves_a_booking_without_a_key`) |
| IDEM-08 (Phase 28) never inspects the first request's response at all — only its SIDE EFFECT (a committed booking, read back via an independent query) is used as ground truth | The actual "lost response" scenario is a proxy or connection dropping the response bytes AFTER the server has already committed — a test that reads the first response and then discards it wouldn't prove anything a proxy-drop couldn't already prove by NOT reading it. Simulating the drop by never looking is closer to the real failure mode than mocking a network layer would be | `tests/bookings/test_idempotency.py` (`test_idem_08_lost_response_retry_shows_the_users_own_existing_booking`) |
| FAIL-01/FAIL-02 (Phase 28) are HTTP-level tests through the real `POST /api/v1/bookings` endpoint, not service-layer calls to `create_booking` directly — extending, not replacing, the existing service-layer proofs (`tests/bookings/test_services.py`, `tests/test_metrics.py`) | Test Plan v1.0's own wording for both ("503 with Retry-After, never a hang," "returns 503, not 500, not 409," "retry with the same idempotency key resolves unambiguously") describes the FULL RESPONSE CONTRACT — status code, header, and idempotent-retry behavior together — which only a real request/response round trip through `run_idempotent_write` and `kairos_exception_handler` can prove. The service-layer tests already prove the SQLSTATE→exception mapping in isolation; Phase 28's job was the layer above it, the same escalation Phase 9 already gave Phase 4's create-booking tests | `tests/test_failure_modes.py` |
| FAIL-01 forces a connection-level failure (`sqlstate=None`) on the FIRST `Booking.objects.create()` call only, via a call-counter closure, rather than monkeypatching every call permanently | A real primary failover is a TRANSIENT condition — the retry must succeed once the "election" is over, which FAIL-01's own wording ("retry with the same idempotency key resolves unambiguously") requires proving, not just that the first attempt fails correctly. A permanent monkeypatch could only prove the first half | `tests/test_failure_modes.py` (`test_fail_01_primary_failover_mid_write_returns_503_then_retry_resolves_unambiguously`) |
| `kairos.core.replica` (Phase 28; FAIL-03) is a NEW, small module — not a stub inside `kairos/resources/views.py` itself, and not real replica infrastructure | Per this phase's own explicit clarification: do NOT stand up a real replica just to test degradation logic; DO prove that logic is correct against a simulated condition. A separate module makes `select_read_source` independently unit-testable (`tests/test_replica.py`) without needing an HTTP request at all, AND gives Phase 30 exactly one function (`current_replica_lag_seconds`) to replace with a real measurement — the routing logic downstream doesn't change | `kairos/core/replica.py` |
| The §10 functional matrix audit (Phase 28) recounted the matrix at 67 rows, not the Test Plan's own "§10 matrix: 100% passing" section header's implied 62 | Verbatim-counted twice against the actual pasted matrix rather than trusting a remembered number — the same "recount rather than assume" discipline Phase 27 already applied to a frontend test-count discrepancy (see that phase's own row below). Reported accurately in `docs/test-plan-compliance.md` rather than silently matching whatever number was expected | `docs/test-plan-compliance.md` |
| Coverage's documented target (90%) is set a few points BELOW the measured baseline (93%), not equal to it | Setting `--cov-fail-under` exactly at the baseline makes CI brittle against any single legitimately-hard-to-cover new branch; setting it at 100% would pressure future phases into writing coverage-only tests against CLI-wrapper plumbing this project's own conventions already reject elsewhere (`expand_occurrences`/`reap_expired_holds`'s "test the mechanism directly, not its thin wrapper" precedent). 90 is deliberate headroom, not an arbitrary round number — see `docs/test-plan-compliance.md`'s own reasoning | `backend/pyproject.toml`, `.github/workflows/ci.yml` |
| RECLAIM-04's own already-recorded 269-deadlock finding (Phase 17) is reported as an OPEN gap in `docs/test-plan-compliance.md`, not silently marked PASS just because Phase 28's own job was to produce a compliance checklist | Test Plan v1.0 §14's literal wording for RECLAIM-04 is "zero deadlocks... a design change requiring re-review, not a tuning fix" — CLAUDE.md's own Phase 17 entry already documents 269 real deadlocks were observed at full scale. Marking this row PASS in a document whose entire purpose is an honest release checklist would defeat that purpose; Phase 28 did not re-run or re-litigate RECLAIM-04 (unchanged, no code this phase touched sits on its path) — it reported the existing finding accurately instead of upgrading its status without new evidence | `docs/test-plan-compliance.md` |
| RECLAIM-04 (Phase 29) is RE-RUN at the identical full scale (200×50) rather than trusted from Phase 17's own number, and the resulting decision is ACCEPT (a known, documented, retryable liveness cost), not a `lock_timeout`/locking-strategy change | Phase 28 left this explicitly open, pending "an explicit design decision" this phase's own instructions required be made with real data, not guessed or deferred again. Re-running first is what turned "should we accept this" into an answerable question — the rate held roughly steady (269 → 221) across four intervening phases of write-path complexity (idempotency, audit, holds, offers, reclamation), evidence this is a stable, bounded characteristic of the exclusion-constraint design under EXTREME (200 simultaneous identical-slot writers) contention, not a growing regression. Safety (never more than one success per round) held perfectly across 20,000 combined raw attempts, both runs — the actual, zero-tolerance guarantee this project cares about was never in question | `docs/performance-baseline.md` (RECLAIM-04 section, full five-point reasoning) |
| PERF-01/02/03/CONC-06 (Phase 29) are throwaway Python scripts under `scripts/perf/`, real HTTP via stdlib `urllib`, not pytest tests | These measure LATENCY under real concurrent load against a real running server — a fundamentally different kind of proof than pytest's correctness assertions (the same "not application code, exists to produce a report" category `scripts/spike/` already established, Phase 1). `threading.Barrier`-released true simultaneity for every "concurrent" measurement mirrors `tests/concurrency/harness.py`'s own discipline, applied to real HTTP instead of raw psycopg, so "200 concurrent requests" in this report means the same thing "200 concurrent writers" already means everywhere else in this codebase | `scripts/perf/common.py` |
| `waitress` and `gunicorn` were installed AD HOC (`.venv`/a throwaway Docker image) purely as load-testing tools, never added to `backend/pyproject.toml`'s committed dependencies | Phase 29's job is to characterize THIS APPLICATION's performance fairly, not to choose Kairos's production server — that choice belongs to Phase 30 (Rollout v1.0 §2.2, still unmade). Getting a fair measurement needed a real multi-threaded server (ruling out "the dev server specifically is bad") and a real multi-process one (`gunicorn`, Unix-only, via a throwaway Linux container reusing the EXISTING `backend/Dockerfile` with its `CMD` swapped, never touching `infra/docker-compose.yml`) — using better tools to MEASURE the app is a different act than picking one to SHIP it with | `docs/performance-baseline.md` ("What was and wasn't installed") |
| CPython's GIL — not just "the dev server" — is the primary driver of this codebase's concurrent-request latency degradation, established by ruling out alternatives one at a time rather than asserted | A single serial request stayed fast (ruling out slow queries); `manage.py runserver` AND `waitress` (a genuinely different, independently-maintained multi-threaded server) showed the IDENTICAL roughly-linear degradation with concurrency (ruling out "this one server implementation is just bad"); only a real multi-PROCESS server (`gunicorn`) substantially improved every number. Three eliminations, not one guess — the same "diagnose, don't assume" discipline this project's own CONC-01 tuning notes (Phase 4/6) already modeled for a different signal | `docs/performance-baseline.md` ("Environment and methodology") |
| `BOOKING_WRITE_P95_ALERT_THRESHOLD_MS` is set to 1500ms — between CONC-06's own N=50 (666ms, still scaling) and N=100 (1760ms, already plateaued) readings, not at either endpoint | Rollout v1.0 §6's own words: "approaching a known ceiling is not a false alarm by definition." A resource whose write P95 has crossed 1500ms is very likely already past the point where adding contention still bought throughput (CONC-06's own inflection point, N≈50) — closer to the plateau side of that transition than the healthy side, deliberately, so the alert fires once a resource looks saturated rather than merely busy | `kairos/core/constants.py`, `docs/performance-baseline.md` |
...
Performance & load testing (Phase 29) — verified live, not estimated, against a real PRD-A1-
scale `kairos_dev` (`manage.py seed_perf_data`, 2,000 users / 301 resources / ~18,800 bookings)
and a real running server. The FIRST attempt used this project's own documented "how to run it
locally" mechanism, `manage.py runserver` — and showed a genuine, diagnosed problem (roughly
linear latency growth with concurrent request count, ruled out as query-slowness by a fast
single serial request, ruled out as "the dev server specifically" by `waitress` — a real,
independent multi-threaded WSGI server — showing the identical pattern). A throwaway Linux
container (the EXISTING `backend/Dockerfile`, `CMD` swapped to real multi-process `gunicorn` for
this measurement only, network-joined to the same `infra_default` Docker network as `postgres`/
`redis`, never touching `infra/docker-compose.yml`, torn down after use) substantially improved
every number and eliminated a second, fully-diagnosed artifact (Windows CPython's `select()`-
based socket multiplexing hitting its own ~512-socket ceiling under `waitress`, unrelated to
Postgres — zero such failures on the Linux/gunicorn rig). All four load-testing scripts
(`scripts/perf/perf_01_booking_write.py`/`perf_02_availability_read.py`/`perf_03_waitlist_
dispatch.py`/`conc_06_escalation.py`) were run to completion against this rig, producing the real
numbers in `docs/performance-baseline.md`; RECLAIM-04 was separately re-run via real `pytest`
(`tests/concurrency/test_reclaim_04.py -v -s`, full 200×50 scale, 373.74s) against the real
`kairos_test` database. Full methodology, every real number, and the explicit RECLAIM-04
accept/reject decision are in `docs/performance-baseline.md` — not reproduced here.
...
`tests/bookings/test_idempotency.py`'s last two tests (Phase 28) — IDEM-07 (a genuine exception
forced at the exact point a process kill mid-write would land, inside `run_idempotent_write`'s
own shared transaction; ground truth confirms neither the booking nor the key survives) and
IDEM-08 (the first response is never inspected at all — simulating a proxy-level drop — only an
independent query's ground truth is used; the retry with the identical key shows the user their
own committed booking, never a 409). `tests/test_failure_modes.py` (Phase 28) — FAIL-01 (a
connection-level failure forced on the FIRST `Booking.objects.create()` call only, via a
call-counter closure, through the real HTTP endpoint: 503, `Retry-After`, `cause="failover"`,
nothing recorded under the key, then a real, unambiguous 201 on retry) and FAIL-02 (genuine
reproduction — a real conflicting row held open on an independent connection, `lock_timeout`
forced short, through the real HTTP endpoint: 503 explicitly not 500 not 409, then an
unambiguous retry once the row releases). `tests/test_replica.py` + two new tests in
`tests/resources/test_views.py` (Phase 28; FAIL-03) — `kairos.core.replica.select_read_source`'s
degradation rule proven directly (unknown/over-threshold lag → `"primary"`, under-threshold →
`"replica"`, the exact boundary) and again through the real `GET /resources/{id}/availability`
endpoint via a monkeypatched `current_replica_lag_seconds()`; the real-infrastructure form is
explicitly deferred to Phase 30, recorded as such in `docs/test-plan-compliance.md`, not claimed
as fully passing. `tests/concurrency/test_conc_01_full_scale.py` (Phase 28; staging tier,
`--ignore`d in CI's `concurrency` job, matching RECLAIM-04's own precedent) — CONC-01's full
100-consecutive-run exercise at N=200 plus the separate N=500 escalation, run manually at full
scale (not merely written) per this project's own established RECLAIM-04-precedent discipline;
real observed numbers are in `docs/test-plan-compliance.md`. Three §10 functional-matrix gaps
closed (a row-by-row audit against the running suite found 62 of 67 rows already covered): the
365-day horizon's true boundary (`tests/bookings/test_views.py`, freezing `timezone.now()` so
`start == now + 365 days` is a deterministic equality, not a 5-minute-inside proxy for it), a
recurring series with zero future occurrences (`tests/bookings/test_recurring_series.py`), and
the six-check response content asserted for an `operations` caller specifically, not only
`system_admin` (`tests/test_admin_checks.py`). Coverage wired into CI via `pytest-cov`
(`--cov=kairos --cov-report=term-missing --cov-report=xml --cov-fail-under=90`, XML uploaded as
a build artifact) — 93% measured baseline at the time this was wired up, 90% documented target;
see `docs/test-plan-compliance.md` for the full reasoning and the two 0%-covered files that are
0% by design (thin CLI wrappers around already-unit-tested functions), not oversight. Full
Test Plan v1.0 §14 hard-blocker checklist: `docs/test-plan-compliance.md` — every item mapped to
its test, including the one item this phase found and reported as an OPEN gap rather than a
silent pass (RECLAIM-04's own already-recorded 269-deadlock finding from Phase 17, Test Plan's
literal "zero deadlocks" wording not currently true) and the two items explicitly out of this
phase's own Scope IN and deferred to Phase 29 (CONC-06a, PERF-01–03 — no realistic-topology/
spike-profile infrastructure exists yet to test either against).

Phase 29's own tests are NOT pytest — `scripts/perf/` (throwaway, like `scripts/spike/`) drives a
real running server over real HTTP, which pytest's own fixture/transaction-rollback model isn't
built for. `manage.py seed_perf_data --reset` seeds PRD A1 scale into `kairos_dev`; `perf_01_
booking_write.py` (steady 5 req/s × 60s + a 200-request barrier-released spike, guaranteed-free
slots on a dedicated empty resource pool so PERF-01's own scope note — "contested latency is
CONC-06's job, not this test's" — holds by construction); `perf_02_availability_read.py`
((a) ordinary density, (b) a near-fully-booked resource at exactly the 92-day bound);
`perf_03_waitlist_dispatch.py` ((a) one isolated cancellation repeated 20x, (b) 50 barrier-
released near-simultaneous cancellations — each measured to the resulting `WaitlistOffer` row's
`created_at`, the closest externally-observable proxy for "notification enqueued" without
instrumenting Celery's broker, documented as a real methodology choice); `conc_06_escalation.py`
(10/25/50/100/250/500 barrier-released writers on one dedicated empty resource, deterministic
non-colliding slot allocation, asserting CONC-06a's hard gate — every response 201 or a
documented 503 — at every step). `tests/test_alerting.py` gained two ordinary pytest tests for
the new `gist_write_throughput` alert (fires over `BOOKING_WRITE_P95_ALERT_THRESHOLD_MS`, reaches
a real email; does not false-alarm under it) — these ARE pytest, unlike the load-testing scripts
above, since they test a code PATH (the alert firing/resolving), not a LATENCY number. Full
non-concurrency suite: 377 tests (375 + 2 new), zero regressions; all CI-tier concurrency tests
re-run clean; RECLAIM-04 re-run manually at full scale (`pytest tests/concurrency/test_reclaim_
04.py -v -s`, 373.74s) for `docs/performance-baseline.md`'s own real numbers.
...
- ~~RECLAIM-04's hard-blocker status is genuinely OPEN, not closed by this phase~~ —
  **resolved by Phase 29**: re-run at the identical full scale (200×50), got current numbers
  (221 deadlocks/22 of 50 runs, down modestly from Phase 17's 269), and made the explicit
  accept/reject decision this entry called for — **ACCEPT the current rate as a known,
  documented, retryable liveness cost; no locking-strategy change before Phase 30** — with the
  full five-point reasoning in `docs/performance-baseline.md`, not deferred a third time.
- ~~CONC-06a and PERF-01 through PERF-03 remain entirely unbuilt~~ — **built by Phase 29**
  (`scripts/perf/`, throwaway load-testing scripts against a real PRD-A1-seeded `kairos_dev`),
  real numbers in `docs/performance-baseline.md`. CONC-06a (the actual hard gate) PASSES
  cleanly (935/935 non-conflicting writes returned 201). PERF-01 spike and PERF-02(b) (near-
  fully-booked, 92-day bound) both FAIL their literal targets — real, reported findings, not
  hidden — but Phase 29's own diagnostic work traced both substantially to this project's
  current single-machine, no-deployment-platform-chosen-yet test environment (see the next
  bullet), not to a correctness defect.
- **FAIL-03's real-infrastructure form is unverifiable until Phase 30 exists** — `kairos.core.
  replica.select_read_source`'s degradation LOGIC is proven (this phase), but nothing in this
  codebase has ever measured real replication lag against a real replica, and Rollout v1.0
  §2.2's own checklist is explicit that standing one up is Phase 30's job, not this phase's or
  any earlier one's. `current_replica_lag_seconds()` is the one function Phase 30 should replace
  — flagged directly in its own docstring, not just here — everything downstream of it needs no
  redesign. Still true after Phase 29 — unchanged.

From Phase 29:

- **PERF-01's spike genuinely FAILS its literal 300ms target under EVERY serving setup tested,
  including a real multi-process container — this is NOT recorded as resolved.** `manage.py
  runserver` (single process): ~7.4s. A throwaway Linux/`gunicorn` container (8 worker processes):
  **3209.8ms — still ~10.7x over target.** Multi-process serving more than halved the number (real,
  substantial evidence CPython's GIL under single-process serving is a DOMINANT contributing
  factor, diagnosed by elimination: a fast serial request ruled out slow queries; `waitress`, a
  real independent multi-threaded server, showing the identical linear-with-concurrency pattern
  ruled out "the dev server specifically is bad") — but 3.2s is not 300ms, and this finding does
  not resolve itself. Two explicit, MANDATORY follow-ups recorded in `docs/performance-baseline.md`,
  not left ambiguous: (1) **Phase 30 must deploy with a real multi-process serving configuration
  (gunicorn/uvicorn with multiple workers, or equivalent) as the ACTUAL production setup** — this
  phase's diagnostic is direct evidence single-process serving is unacceptable for this codebase's
  write path, regardless of which specific software Phase 30 ultimately picks; (2) **PERF-01's
  spike case must be RE-RUN against that real deployment** before this target can be honestly
  marked PASS or FAIL for release — neither claim is defensible from this phase's synthetic,
  single-machine data alone.
- **PERF-02(b) (near-fully-booked/92-day-bound read, P95 1.9s vs. 500ms) has a real, NON-
  environmental cause, unlike PERF-01's spike** — a single SERIAL request against the same dense
  resource completes in 130–200ms, so the cost under concurrency is Python-side: building and
  JSON-serializing a ~2,000-entry `busy_blocks` array once per request. A bigger/better-deployed
  server reduces this proportionally (the same way it helped PERF-01) but does NOT eliminate it —
  re-running this one against Phase 30's real deployment will not make the finding moot the way it
  might for PERF-01. **Explicit decision recorded** (`docs/performance-baseline.md`, four reasons
  in full): **ACCEPT for go-live as a documented, SCOPED limitation** (it needs BOTH a densely-
  booked resource AND a full-92-day query at once — PERF-02(a), the everyday-query shape, passes
  with real margin; no correctness/safety guarantee is at risk, only read latency on a worst-case
  query; the concrete fix, pagination on `busy_blocks`, is a real Spec v1.0 §5.7 contract change
  needing its own design pass, not a same-day patch) — but flagged **HIGH PRIORITY, not deferred
  indefinitely**, given RFC v1.0 §7.2's own framing of availability reads as this system's
  highest-traffic endpoint: recommended BEFORE Phase 30's go-live sign-off if any real resource is
  expected to approach this booking density early, and as an immediate fast-follow after Phase 30
  otherwise. Named cause, named fix, named priority — the one thing intentionally not assigned is
  a phase number, since inventing one wasn't this phase's call.
- **The GiST write-throughput alert threshold (`BOOKING_WRITE_P95_ALERT_THRESHOLD_MS=1500`) is
  explicitly PROVISIONAL** — derived from ONE synthetic, single-machine CONC-06 escalation run,
  exactly the "Stage 2 onward... validates or revises" number Rollout v1.0 §9's own table already
  anticipates being revisited once real production traffic exists to compare against. Not a
  guess (CONC-06's own data set it, per this phase's explicit instruction), but also not meant to
  be read as final. Whichever phase first has real production `RequestMetric` data should
  re-examine this constant specifically, not assume it was ever meant to be permanent.
