...
│                          # src/layout/ (AppLayout, NavBar), src/pages/ (DashboardPage
│                          # placeholder, NotFoundPage) — booking/resource/calendar screens
│                          # are a later phase. Tailwind v4 (CSS-first @theme). Vitest + React
│                          # Testing Library (src/api/client.test.ts — idempotency-key reuse,
│                          # 503 retry, error.code branching, all unit-tested directly)
...
- **`frontend/`'s routing has exactly one authenticated page (`DashboardPage`, a
  placeholder)** — booking creation, resource browsing/availability, waitlist, and admin
  screens all have zero frontend code yet. Phase 23's own scope was explicitly the shell
  (auth, routing, the API client, layout) that later phases build screens into, not the
  screens themselves — see Implementation Plan for which phase(s) own which screen.
