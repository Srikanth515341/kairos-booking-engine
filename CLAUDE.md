...
None. Phase 28 is complete pending review and merge. Phase 29 is next.
...
- **RECLAIM-04's hard-blocker status is genuinely OPEN, not closed by this phase** — Test Plan
  v1.0 §14 requires "zero deadlocks... failure here is a design change requiring re-review, not
  a tuning fix" for release, but Phase 17's own already-recorded, real result (200 writers × 50
  runs) was 269 genuine SQLSTATE 40P01 deadlocks, roughly half the runs affected. Phase 28's own
  Scope IN never named RECLAIM-04 — it was neither re-run nor re-designed this phase — but
  `docs/test-plan-compliance.md` reports its status accurately rather than defaulting every row
  to PASS just because a compliance checklist was the deliverable. **Whichever phase owns
  release readiness (Phase 30, most likely) needs to make an explicit decision here**: tune
  `lock_timeout`/the write path's contention behavior until a re-run genuinely reaches zero, or
  make and document a deliberate call that the observed liveness rate (never a safety violation
  — never more than one success per round, across all 10,000 attempts) is within acceptable
  tolerance for release. Neither decision was this phase's to make.
- **CONC-06a and PERF-01 through PERF-03 remain entirely unbuilt** — explicitly outside this
  phase's own Scope IN (which named CONC-01/IDEM-07-08/FAIL-01-03/§10/coverage only), and
  CLAUDE.md's own "NOT Yet Built" section already deferred CONC-06 to Phase 29 before this phase
  began. PERF-01-03 need the identical realistic-connection-topology/spike-profile
  infrastructure CONC-06 does (Test Plan v1.0 §13's own "realistic data volume... PRD A1 scale"
  environment requirement) — deferring them alongside it, to the same phase, rather than
  building a second, incompatible measurement harness, is the honest reading of that shared
  dependency. Whichever phase builds CONC-06 should build PERF-01-03 in the same pass.
